        ...
        <div class="col-lg-8">
            ...
            <div class="card main-card">
                <div class="card-header text-center">
                    <h2 class="mb-0">วาสุเทพ ดวงแพงมาตร (โอเว่น) </h2>
            ...
            <?php if(isset($_POST['Submit'])): ?>
                <?php
                    ...
                    // หมายเหตุ: ตรวจสอบไฟล์ connectdb.php ว่ามีอยู่จริงและตั้งค่าถูกต้อง

                    // รับค่าและป้องกัน SQL Injection
                    $fullname = mysqli_real_escape_string($conn, $_POST['fullname']);
                    $phone    = mysqli_real_escape_string($conn, $_POST['phone']);
                    $saraly   = mysqli_real_escape_string($conn, $_POST['saraly']);
                    $height   = mysqli_real_escape_string($conn, $_POST['height']);
                    $color    = mysqli_real_escape_string($conn, $_POST['color']);
                    $major    = mysqli_real_escape_string($conn, $_POST['major']);
                    
                    // คำสั่ง SQL (แก้ไข Syntax ให้ถูกต้อง)
                    $sql = "INSERT INTO register (r_id, r_name, r_phone, r_saraly, r_height, r_color, r_major) 
                            VALUES (null, '$fullname', '$phone', '$saraly', '$height', '$color', '$major')";
                    
                    mysqli_query($conn, $sql) or die ("Insert ข้อมูลไม่ได้: " . mysqli_error($conn));
                ...
                <div class="alert alert-success mt-4 shadow-sm" role="alert">
                    ...
                    <p>ยินดีต้อนรับคุณ <strong><?php echo htmlspecialchars($fullname); ?></strong> เข้าสู่ระบบ</p>
                    <hr>
                    <div class="row">
                        <div class="col-md-6">
                            <ul class="list-unstyled mb-0">
                                <li><strong>เบอร์โทร:</strong> <?php echo htmlspecialchars($phone); ?></li>
                                <li><strong>สาขาวิชา:</strong> <?php echo htmlspecialchars($major); ?></li>
                                <li><strong>เงินเดือน:</strong> <?php echo number_format($saraly); ?> บาท</li>
                            </ul>
                        </div>
                        <div class="col-md-6">
                            <ul class="list-unstyled mb-0">
                                <li><strong>ความสูง:</strong> <?php echo htmlspecialchars($height); ?> ซม.</li>
                                <li class="d-flex align-items-center">
                                    <strong>สีที่ชอบ:</strong> 
                                    <span style="margin-left:5px;"><?php echo htmlspecialchars($color); ?></span>
