    ...
    <div class="row justify-content-center">
        <div class="col-lg-9">
            ...
            if(isset($_POST['submit_application'])){
                ...
                include_once("connect_jobapplicationdb.php");
                ...
                // ตรวจสอบค่าว่างสำหรับ skills และ experience
                $skills_raw = $_POST['skills']; 
                $skills = empty($skills_raw) ? "-" : mysqli_real_escape_string($conn, $skills_raw);
                
                $experience_raw = $_POST['experience'];
                $experience = empty($experience_raw) ? "ไม่มีประสบการณ์" : mysqli_real_escape_string($conn, $experience_raw);

                // 3. เขียนคำสั่ง SQL บันทึกลงตาราง applications
                $sql = "INSERT INTO applications (position, title_name, fullname, dob, education, skills, experience) 
                        VALUES ('$position', '$title_name', '$fullname', '$dob', '$education', '$skills', '$experience')";

                // 4. สั่งรันคำสั่ง SQL
                $result = mysqli_query($conn, $sql) or die ("บันทึกข้อมูลไม่ได้: " . mysqli_error($conn));

                // คำนวณอายุเพื่อแสดงผล
                ...
                if($result){
                    ...
                    <div class="alert alert-success mt-4 shadow-sm fade show" role="alert">
                        ...
                        <p>ขอบคุณที่สนใจร่วมงานกับเรา ข้อมูลถูกบันทึกลงฐานข้อมูลเรียบร้อยแล้ว</p>
                        <hr>
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <strong>ความสามารถพิเศษ:</strong><br> <small class="text-muted">'.nl2br($skills).'</small><br>
                                <strong>ประสบการณ์:</strong><br> <small class="text-muted">'.nl2br($experience).'</small>
                            </div>
                        </div>
                    </div>
                    ';
